...
ERROR_PATTERNS = [
    ...
    (re.compile(r"No space left on device", re.IGNORECASE), "NO_SPACE"),
]
...
def main():
    ...
    no_space_errors = [e for e in errors if e[1] == "NO_SPACE"]
    if no_space_errors:
        recs.append(
            "'No space left on device' error detected.\n"
            "      ReaR uses /tmp (or the directory set by TMPDIR) as working space\n"
            "      when building the rescue image and backup archive.\n"
            "      Suggested remediation steps:\n"
            "        1. Free up space in /tmp:  sudo rm -rf /tmp/rear.*\n"
            "        2. Check overall disk usage: df -h /tmp /var /\n"
            "        3. If /tmp is a tmpfs, increase its size:\n"
            "             sudo mount -o remount,size=2G /tmp\n"
            "           or add to /etc/fstab:  tmpfs /tmp tmpfs defaults,size=2G 0 0\n"
            "        4. Redirect ReaR's scratch dir to a larger filesystem by adding\n"
            "             BUILD_DIR=/path/with/enough/space\n"
            "           to /etc/rear/local.conf and re-run the backup."
        )
